...
class HumongousTrees:
    ...
    def parse_dna(raw_dna):
        ...
        parsed = {}
        test = {}
        for line_no, block in enumerate(raw_dna, start=1):
    ...
    def receivable_energy(self, stems):
        ...
        return total_energy
    def build_tree(self, num_years = 100, visualize=False):
        sprouts = {(1, 0): (1, "00")}
        stems = set()
        ...
        for year in range(1, num_years + 1):
            ...
            energy_avail = self.receivable_energy(set(stems))
            # print(year, avail_biomass, energy_reqd, energy_avail)
            if (energy_reqd >= energy_avail) and year > 5:
                break
...
class HumongousTrees:
    ...
    def build_tree(self, num_years=100, visualize=False):
        sprouts = {(1, 0): "00"}
        stems = set()
        avail_biomass = 1  # single starting sprout, in case of an immediate edge case
        ...
        for year in range(1, num_years + 1):
            ...
            candidates = defaultdict(list)  # coord -> list of (dna_id, line_no) candidates
            for (height, col), dna_id in sprouts.items():
                growth = self.dna_dict[dna_id]
                ...
                for direction, new_id in growth.items():
                    ...
                    if new_coord in stems:
                        continue  # blocked: a stem already permanently occupies this cell
                    candidates[new_coord].append(new_id)

            new_sprouts = {}
            for coord, entries in candidates.items():
                winner_id = max(entries, key=lambda e: int(e))
            ...
            if energy_reqd >= energy_avail and year >= 5:
                break
        print(year, avail_biomass, energy_avail, energy_reqd)
        return avail_biomass
...
for raw in data:
    raw_lines = raw.split("\n\n") if "\n\n" in raw else [raw]
    tree = HumongousTrees(raw_lines)
    total_biomass += tree.build_tree()

print("FlipFlop 2026, Puzzle 11")
print("Part 1:", total_biomass)
